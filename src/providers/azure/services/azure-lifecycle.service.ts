...
export async function fetchAzureLifecycleLookup(): Promise<AzureLifecycleLookup> {
  ...
  if (prevGenMd) {
    ...
    logger.warn(
      'Could not fetch online previous-gen-sizes.md; using official Microsoft Learn baseline lookup.',
    );
  }
  ...
  if (retiredMd) {
    ...
    logger.warn(
      'Could not fetch online retired-vms.md; using official Microsoft Learn baseline lookup.',
    );
  }
